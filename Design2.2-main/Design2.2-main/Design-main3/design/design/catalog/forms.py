...
class ChangeStatusRequest(forms.ModelForm): #изменение статуса заявки
    ...
    def clean(self):
        ...
        if new_status == 'Выполнено':
            ...
            if not img:
                raise forms.ValidationError("Прикрепитe изображение дизайна")

        if new_status == 'Принято в работу':
            comment = cleaned_data.get('comment')
            if not comment:
                raise forms.ValidationError("Укажите комментарий")
